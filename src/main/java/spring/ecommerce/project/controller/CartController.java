...
@RestController //combination of @Controller and @ResponseBody(which returns JSON body)
...
public class CartController {
    ...
    @PostMapping("/add")
    public CartItem addToCart(@RequestHeader("Authorization") String authorization, @Valid @RequestBody CartRequest request) {
    ...
    @PutMapping("/{itemId}")
    public CartItem updateQuantity(@RequestHeader("Authorization") String authorization, @PathVariable Long itemId, @Valid @RequestBody UpdateQuantityRequest request) {
        UserEntity user = authService.requireUser(authorization);
