...
async function api(path, method = "GET", body = null, auth = false) {
    ...
    if (auth && token)
        headers["Authorization"]=`Bearer ${token}`;//this connects to AuthService.requireUser();
...
//fetch user details and set session after login/registration
...
async function placeOrderWithPayment() {
    ...
    if (!order.id) { showToast("Failed to place order. Your cart is Empty."); return; }
